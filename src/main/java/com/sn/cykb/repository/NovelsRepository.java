...
public interface NovelsRepository extends JpaRepository<Novels, String> {
    ...
    @Query(value = "select * from novels where source_name = ?1 and create_time < ?2 order by create_time desc limit ?3", nativeQuery = true)
    List<Novels> findMoreClassifyNative(String sourceName, Long createTime, int size);
